            ...
            for _, row in df.iterrows():
                try:
                    ...
                    if len(entry) == len(cfg["cols"]):
                        temp_storage[game_type].append(entry)
                except: continue
        except: continue
...
def render_prediction_row(nums, counts, special_num=None):
    html_list = ['<div style="display:flex;justify-content:center;flex-wrap:wrap;">']
    for n in nums: html_list.append(get_ball_html(n, counts.get(n, 1)))
    if special_num is not None: html_list.append(get_ball_html(special_num, 1, True))
...
# --- 8. 介面主程式 ---
with st.sidebar:
    st.title("🎛️ 總控中心 v26.3")
    if st.button("🔄 掃描並重整資料庫 (Fix All)", type="primary"):
        with st.spinner("正在重整資料庫..."):
            files, stats = rebuild_databases()
            load_db_data.clear()
            st.success(f"重整完成！掃描 {files} 個檔案。")
    ...
    st.markdown("---")
    uploaded_files = st.file_uploader("補檔案 (CSV/ZIP)", accept_multiple_files=True, type=['csv', 'zip'])
    if uploaded_files:
        if st.button("📥 匯入"):
            with st.spinner("處理中..."):
                bar = st.progress(0)
                process_bulk_files(uploaded_files, bar)
                bar.empty()
                st.toast("檔案已上傳，建議執行「重整」")
                st.success("匯入完成！")
...
st.title(f"🎯 {selected_game} 數據中心")

if df.empty:
    st.warning("⚠️ 資料庫空白。")
    st.info("請確認 data 資料夾內有 CSV 檔，並按下左側的 **「🔄 掃描並重整資料庫」** 按鈕。")
else:
    st.caption(f"資料範圍: {df.iloc[0]['Date']} ~ {df.iloc[-1]['Date']} (共 {len(df)} 期)")
    num_cols = [c for c in cfg["cols"] if c.startswith("N")]
    df_nums = df[num_cols].apply(pd.to_numeric)
    ...
    with tab1:
        ...
        if st.button("🎲 啟動運算", type="primary"):
            candidates = []
            templates = [{"name": f"狙擊 ({sniper_mode})", "mode": sniper_mode}, {"name": "順勢 (Trend)", "mode": "trend"}, {"name": "版路 (Drag)", "mode": "banlu"}]
            sp_probs = None
            ...
            for i, temp in enumerate(templates * 2):
                ...
                while not found and att < 5000:
                    ...
                    if abs(curr_std - avg_std) <= tol:
                        sp_num = None
                        if sp_probs is not None: sp_num = np.random.choice(sp_probs.index.tolist(), p=sp_probs.values)
        ...
        if f'last_candidates_{selected_game}' in st.session_state:
            results = st.session_state[f'last_candidates_{selected_game}']
            if results:
                all_nums = []
